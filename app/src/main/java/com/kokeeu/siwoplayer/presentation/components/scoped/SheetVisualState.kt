package com.kokeeu.siwoplayer.presentation.components.scoped
...
internal fun rememberSheetVisualState(
    ...
    val baseBottomPadding = remember(containerHeight, sheetCollapsedTargetY, density) {
        ...
        (containerHeight - com.kokeeu.siwoplayer.presentation.components.MiniPlayerHeight - targetYDp)
            .coerceAtLeast(0.dp)
    }

    val currentBottomPadding = 0.dp

    val miniHeightPx = remember(density) { with(density) { com.kokeeu.siwoplayer.presentation.components.MiniPlayerHeight.toPx() } }
