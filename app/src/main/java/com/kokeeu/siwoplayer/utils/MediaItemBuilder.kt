...
object MediaItemBuilder {
    ...
    private const val EXTERNAL_EXTRA_PREFIX = "com.kokeeu.siwoplayer.external."
    ...
    fun buildForExternalController(context: Context, song: Song): MediaItem {
        ...
        return com.kokeeu.siwoplayer.data.diagnostics.PerformanceMetrics.time(
            com.kokeeu.siwoplayer.data.diagnostics.PerformanceMetrics.Timings.MEDIASESSION_ITEM_BUILD
        ) {
            MediaItem.Builder()
                .setMediaId(song.id)
                .setUri(playbackUri(song))
